...
let discover_camlp5_dir cfg =
  String.trim @@ Cfg.Process.run_capture_exn cfg "ocamlfind" [ "query"; "camlp5" ]
;;
...
let discover_gt_flags cfg =
  let gt_archives =
    Cfg.Process.run_capture_exn
      cfg
      "ocamlfind"
      [ "query"; "-pp"; "camlp5"; "-a-format"; "-predicates"; "byte"; "GT,GT.syntax" ]
  in
  ...
  let gt_archives_native =
    Cfg.Process.run_capture_exn
      cfg
      "ocamlfind"
      [ "query"; "-pp"; "camlp5"; "-a-format"; "-predicates"; "native"; "GT,GT.syntax" ]
  in
...
let discover_logger_flags cfg =
  ...
  *)
  ...
  let cmos =
    ...
    |> List.map (fun file ->
           if Filename.basename file = pr_o_cmo
           then Filename.concat camlp5_dir pr_o_cmo
           else if Filename.basename file = pr_dump_cmo
           then Filename.concat camlp5_dir pr_dump_cmo
           else file)
  in
